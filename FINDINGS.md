...
| 2 | 🟡 | backend/models/productModel.js | нет min/max в схемах — price/countInStock можно передать отрицательными, rating без диапазона | добавить `min: 0` для price/countInStock, `min: 1, max: 5` для rating в reviewSchema | 🔴 not yet |
